...
Running `./install.sh` without an option starts the same installation flow.
...
The executable install workflow also writes a color-free diagnostic log to a unique file under `$XDG_STATE_HOME/my-omarchy-config/logs/`, or under `$HOME/.local/state/my-omarchy-config/logs/` when `XDG_STATE_HOME` is empty or relative. Log directories use mode `0700` and log files use mode `0600`; sourcing the installer and displaying help do not create logs.

The installer is split by responsibility: `lib/runtime.sh` owns state, logging, and final status; `lib/config.sh` owns backups and links; `lib/samba.sh` owns privileged Samba setup; and `lib/modules.sh` owns the package/config wizard.
